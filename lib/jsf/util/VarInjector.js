  ...
  _createClass(VarInjector, null, [
    {
      ...
      value: function inject(component, varName, object) {
        ...
        for (var key in component.props) {
          ...
          if (key === 'children' &&
              _typeof(component.props[key]) === 'object') {
            continue;
          }
        ...
        if (component.hasOwnProperty('props') &&
            ...
            _typeof(component.props.children) === 'object') {
          // add key properties otherwise react complains
          _react2.default.Children.forEach(component.props.children,
              function(child, i) {
                children.push(VarInjector.inject(
                    _react2.default.cloneElement(child, {key: i}), varName,
                    object));
              });
        } else {
      ...
      value: function replace(string, varName, object) {
        ...
        try {
          for (var _iterator = string.match(
              regex)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            var group = _step.value;

            //group has the pattern #[varName..*], extract only .*
            var property = group.slice(2 + varName.length + 1, group.length -
                1);
